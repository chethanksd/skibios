...
volatile task_t    KSECTION(.kbss) *task_obj[MAX_TASK_COUNT];
volatile uint32_t  KSECTION(.kbss) PSP_Array[MAX_TASK_COUNT];
volatile uint8_t   KSECTION(.kbss) priority_Array[MAX_TASK_COUNT][2];
volatile uint32_t  KSECTION(.kbss) mutex_stash[MAX_TASK_COUNT];
volatile uint8_t   KSECTION(.kbss) level_stash[MAX_TASK_COUNT];
volatile int8_t    KSECTION(.kbss) jmp_list[MAX_TASK_COUNT];
volatile uint8_t   KSECTION(.kbss) state[MAX_TASK_COUNT];
volatile uint32_t  KSECTION(.kbss) task_id[MAX_TASK_COUNT];
volatile uint16_t  KSECTION(.kbss) permissions[MAX_TASK_COUNT];

volatile uint32_t  KSECTION(.kbss) *op1[MAX_TASK_COUNT];
volatile uint32_t  KSECTION(.kbss) *op2[MAX_TASK_COUNT];
volatile uint32_t  KSECTION(.kbss) hib_value[MAX_TASK_COUNT];
